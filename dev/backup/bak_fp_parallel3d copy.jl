...
function fp_parallel3d_kernel!(p, img, vectors, iangles, nx, ny, nz, c0, c1, c2, dir, scale1, scale2, out_scale)

    # project for one angle
    tid = threadIdx().x + (blockIdx.().x - 1) * blockDim().x
    niangles = length(iangles)
    nrow, ncol, nangles = size(p)
    I = CartesianIndices((1:nrow, 1:ncol, 1:niangles))

    # for each detector pixel
    @inbounds if tid <= length(I)
        detV, detU, idx_angles = Tuple(I[tid])
        iang = iangles[idx_angles]

        #------------------
        # compute detSX

        detSX = vectors[iang, 4] -0.5f0*vectors[iang, 7]*ncol -0.5f0*vectors[iang,10]*nrow
        
        detSY = vectors[iang, 5] -0.5f0*vectors[iang, 8]*ncol -0.5f0*vectors[iang,11]*nrow
        
        detSZ = vectors[iang, 6] -0.5f0*vectors[iang, 9]*ncol -0.5f0*vectors[iang,12]*nrow

        # detSX /= vol_geom.spacingX
        # detSY /= vol_geom.spacingY
        # detSZ /= vol_geom.spacingZ

        detSX += 0.5f0*vectors[iang, 7] + 0.5f0*vectors[iang, 10]
        detSY += 0.5f0*vectors[iang, 8] + 0.5f0*vectors[iang, 11]
        detSZ += 0.5f0*vectors[iang, 9] + 0.5f0*vectors[iang, 12]   

        #------------------
        # compute detX for detU, detV

        detX = detSX + (detU-1)*vectors[iang, 7] + (detV-1)*vectors[iang, 10]
        detY = detSY + (detU-1)*vectors[iang, 8] + (detV-1)*vectors[iang, 11]
        detZ = detSZ + (detU-1)*vectors[iang, 9] + (detV-1)*vectors[iang, 12]

        a1 = c1(vectors[iang, 1], vectors[iang, 2], vectors[iang, 3]) / c0(vectors[iang, 1], vectors[iang, 2], vectors[iang, 3])
        a2 = c2(vectors[iang, 1], vectors[iang, 2], vectors[iang, 3]) / c0(vectors[iang, 1], vectors[iang, 2], vectors[iang, 3])
...
function fp_parallel3d!(p, img, proj_geom, vol_geom)
    ...
    vecs = cu(proj_geom.Vectors) # copy 
    ...
    # (translate) and scale vectors
    vecs[[1,7,10],:] ./= vol_geom.spacingX
    vecs[[2,8,11],:] ./= vol_geom.spacingY
    vecs[[3,9,12],:] ./= vol_geom.spacingZ

    # pre-determine the dominant direction first
    dir_y = (abs.(vecs[:,1]) .>= abs.(vecs[:,2])) .* (abs.(vecs[:,1]) .>= abs.(vecs[:,3]))
    iangles_xdir = cu(findall(.~dir_y))
    iangles_ydir = cu(findall(dir_y))

    # will call CUDA two times for x, y-dir.
    
    # call for x-dir dominant case
    c0 = (x,y,z) -> x; c1 = (x,y,z) -> y; c2 = (x,y,z) -> z
    nblocks = cld(size(p,1)*size(p,2)*length(iangles_xdir), nthreads)
    dir = 0
    out_scale = vol_geom.spacingX
    scale1 = (vol_geom.spacingY / vol_geom.spacingX)^2
    scale2 = (vol_geom.spacingZ / vol_geom.spacingX)^2

    @time @cuda threads=nthreads blocks=nblocks fp_parallel3d_kernel!(p, img_texture, vecs, iangles_xdir, nx, ny, nz, c0, c1, c2, dir, scale1, scale2, out_scale)

    c0 = (x,y,z) -> y; c1 = (x,y,z) -> x; c2 = (x,y,z) -> z
    nblocks = cld(size(p,1)*size(p,2)*length(iangles_ydir), nthreads)
    dir = 1
    out_scale = vol_geom.spacingY
    scale1 = (vol_geom.spacingX / vol_geom.spacingY)^2
    scale2 = (vol_geom.spacingZ / vol_geom.spacingY)^2
    @time @cuda threads=nthreads blocks=nblocks fp_parallel3d_kernel!(p, img_texture, vecs, iangles_ydir, nx, ny, nz, c0, c1, c2, dir, scale1, scale2, out_scale)
...
#         unction fp_parallel3d_kernel!(p, img, vectors, iangles,
